...
class DashboardComponent extends Component {
  ...
  componentDidMount = async () => {
    ...
    if (symbolKeys.length > 0) {
      for (var i in symbolKeys) {
        ...
        this.mountedSubscriptions[symbolKey] = API.graphql(
          graphqlOperation(subscriptions.intradayStockPriceCreated, {
            symbol: symbolKey,
          })
        ).subscribe({
  ...
  handleSubscription(data) {
    ...
    if (
      dataPoints[symbol].points.length == 0 &&
      this.buffer[symbol].length == 0
    ) {
      this.buffer[symbol].push(point);
      isAddedToBuffer = true;
    } else if (this.buffer[symbol].length > 0) {
      dataPoints[symbol].points = dataPoints[symbol].points.concat(
        this.buffer[symbol]
      );
      this.buffer[symbol] = [];
    }
  ...
  loadDashboard = async () => {
    ...
    for (var i in symbols) {
      ...
      dataPointsBySymbol[symbolText] = await this.retrieveLatestPricesBySymbol(
        symbolText
      );
    }
  ...
  retrieveLatestPricesBySymbol = async (symbol) => {
    ...
    do {
      var results = await API.graphql(
        graphqlOperation(queries.retrieveLatestIntradayPrices, {
          symbol: symbol,
          ts: 0,
          nextToken: nextToken,
        })
              ...
              <div className="m-2">
                ...
                <div className="border">
                  {hasDataPoints ? (
                    <Chart
                      symbol={symbol}
                      data={this.state.dataPoints[symbol].points}
                    />
                  ) : (
                    <div className="p-2">
                      <h5 className="text-danger">No data points</h5>
                    </div>
                  )}
                </div>
              </div>
